...
delta_teta = np.array(np.deg2rad(df['delta_teta'][1:]))
time = np.array(df['Time'])
phi = np.array(df['Orientation'])
delta_t = (time[1:]- time[:-1])/1000

# print(delta_t*1000)

# print(phi_dot[1:]-phi_dot[:-1])
...
def equations_of_motion(x, y, x_dot, y_dot, teta, delta_t, f_c, tau_f ,phi , phi_dot):
    ...
    y_acc = -(f_c/M) * np.sin(teta) #because tate is negative
...
x_new, y_new, phi_new ,x_dot_new , y_dot_new , phi_dot_new = equations_of_motion(pos_x[-1], pos_y[-1],
                                                              x_dot_new, y_dot_new, teta[0], delta_t[0],
                                                              f_c_opt, tau_f_opt, phi , phi_dot)
    # optimized_path_x.append(x_new)
    # optimized_path_y.append(y_new)
# optimized_phi.append(phi_new)
optimized_phi =phi_new
#
print(len(phi))
print('optimized',len(optimized_phi))
print(optimized_phi)
...
plt.plot(time, phi, linestyle='-', color='b', label='Original Phi')
# plt.plot(time, optimized_phi, linestyle='-', color='g', label='Optimized Phi')
plt.xlabel('Time')
plt.ylabel('Phi (Orientation)')
plt.title('Original Phi vs. Optimized Phi')
plt.legend()
plt.grid()
plt.show()
